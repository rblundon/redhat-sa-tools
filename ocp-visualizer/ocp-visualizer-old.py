...
def get_file_creation_date(clusterId):
    try:
        creation_time = os.path.getctime(clusterId)
...
def process_cluster_data(cluster_id):
    ...
    logging.info(f"Opening file: {cluster_id}")
    try:
        with open(cluster_id, mode='r', newline='', encoding='utf-16') as csvfile:
        ...
        logging.error(f"File not found: {cluster_id}")
    except Exception as e:
        logging.error(f"Error reading file {cluster_id}: {e}")
...
def main():
    ...
    for cluster, value in clusterInfo.items():
        ...
        # Update file paths to use the input file's directory
        cluster_file_path = os.path.join(input_file_dir, cluster + ".csv")

        logging.info(f"Opening file: {cluster_file_path}")
        if cluster_file_exists(cluster_file_path):
            ...
            file_date = get_file_creation_date(cluster_file_path)
            logging.info(f"Date of data for cluster is: {file_date}")

            node_info = process_cluster_data(cluster_file_path)
