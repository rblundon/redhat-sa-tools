...
def parse_args():
    ...
    parser.add_argument("-f", "--file", type=str, help="Input file path (required unless --generate-images is used alone)")
    ...
    if not args.generate_images and not args.file:
        parser.error("the following arguments are required: -f/--file (unless using --generate-images alone)")
